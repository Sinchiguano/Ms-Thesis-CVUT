...
Two type of experiment is done in order to evaluate the pose estimation pipeline when it comes to accuracy and robustness as good properties. As described in \ref{chap:theo}, two 3D datas are required. One of them, a point cloud generated from the CAD model, and the other one, a point cloud which represents the scene, taken from the sensory device used in turn(Astra or RealSense camera). The point cloud generated from the CAD model is called source cloud and the point cloud generated from the depth image is called the target cloud in this thesis.\\
The first experiment will consider the two points as described previously. As to the second experiment, a new source cloud is generated with the camera used in turn.  
