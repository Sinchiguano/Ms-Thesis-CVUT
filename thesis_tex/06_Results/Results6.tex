...
This section presents the experiments and the results of the validation test applied to the 3D object pose estimation system. But for such experiment to be accomplished, few requirements need to be met.The first requirement, a 3D industrial object, namely a textured object is available. As the second requirement, its CAD model is also available since it was created by the author of this thesis with the use of the FreeCAD software \ref{freecadb}. And last but not least, the third requirement is related to the pose of the object to be known in advance. The last requirement is with the aim to be able to compare the output of the pose estimation pipeline with the pose knowing a priori. By comparing both pose of the object, an error is calculated. But it was proved to be difficult to determinent the pose of the object also called ground truth. \\
In order to get the ground of the object, several methods were discussed. And a fast solution was found. The solution has to be with the use of TCP of the robot for localizing object. Such capability of the robot for licalizing object given the fact that tree points of the object are determined. The pose of the object to be localized is regular checkerboard with knowing dimensions. The whole setup for the experiment can be seen in Figure \ref{fig:setup2.jpg} where the checkerboard object is licalized onto the top of the table. Figure \ref{fig:setup2.jpg} shows a rviz simulation, where the relationship between transformation are clearly seen over time. 
...
\iffalse
, then, a suitable scaling undergoes with the use of CluodCompare software \ref{cloudcompareb}, where a point cloud is generated. As to the validation of the method, a ground truth of the object needs to be known in advance. For such a requirement. A checkboard is used as described in Figure \ref{setupsystem}. Then, by placing the robot TCP to the specific points (three points in totally), the checkerboard can be localized. With that, a new workspace is produced. By analyzing the translation and rotation errors, the system is evaluated.\\
The checkboard workspace is used for a rough estimation of the object's pose which is compared with 3D object pose estimation system described in \ref{chap:theo}
\fi