...
\item  For the case when when the source cloud as well as the target cloud are generated from the sensory device as described in \ref{valitest}. The Figure \ref{fig:errorx_2nd}, \ref{fig:errory_2nd}, and \ref{fig:erroryaw_2nd} where obtained for such experiment. It can bee seen that the estimated pose deviates slightly from its ground values.
...
\item From the deviation graph an absolute error can be calculated. Figure \ref{absx}, \ref{absy} and \ref{absyaw} show the error calculated for both experiments.
...
valuations of the system when using only one model point cloud of the LS5
were also performed, and are presented in table 4.3. The single point cloud was
rendered from the side, so no top nor bottom were captured. Otherwise the same
parameters as in table 4.2 were used.
