...
In Table \ref{absoluteR}, the absolute error values calculated for RealSense D-415 are shown. It clearly shows a huge improvement with the new camera. It reports an overall mean value for the x-axis of \unit[0.009] {cm},  with standard deviation of \unit[0.001] {cm}. For the y-axis reported an overall mean error of \unit[0.001] {cm} with a standard deviation of \unit[0.001] {cm}. As to the orientation around the z-axis, an overall mean of $0.698^{\circ}$ with a standard deviation of $0.400^{\circ}$ was reported. It was proved satisfactory that the pose estimation system performed successfully as long as the source cloud and target cloud are generated from the same sensory device which is our with the RealSense D-415 camera. 
...
The results showed above were sastifactory, given enough confident to execute one more experiment, where a small database of source clouds (partial view of the object) was generated from the new camera (RealSense D-415). Figures \ref{fig:mydataset} shows the RGB images of the dataset for better understanding. By defining the dataset, the pose estimation system was evaluated by calculating two metrics: fitness, which measures the overlapping area, and the Root Mean Square Error, which measures the standard deviation of the residuals. The results were satisfactory and acceptable for this type of task provided that between measurements different move were executed according to the dataset created. Figure \ref{new1} shows the results of the pose estimation system.
